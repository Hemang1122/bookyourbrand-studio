...
// ─── Email Templates ─────────────────────────────────────────────────────────
const notificationEmailTemplate = (recipientName: string, message: string, url: string) => `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
const getEmailSubject = (type: string, message: string): string => {
  ...
  if (message.toLowerCase().includes('project')) return '📁 Project Update — BookYourBrands Studio';
  if (message.toLowerCase().includes('task')) return '✅ Task Update — BookYourBrands Studio';
  if (message.toLowerCase().includes('package')) return '📦 Package Update — BookYourBrands Studio';
